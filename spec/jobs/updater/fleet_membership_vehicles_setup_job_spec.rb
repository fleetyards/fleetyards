...
RSpec.describe Updater::FleetMembershipVehiclesSetupJob do
  describe "#perform" do
    it "calls setup_fleet_vehicles on the fleet membership" do
      user = create(:user)
      fleet = create(:fleet, members: [user])
      fleet_membership = fleet.fleet_memberships.first

      allow(fleet_membership).to receive(:setup_fleet_vehicles)
      ...
      expect(fleet_membership).to have_received(:setup_fleet_vehicles)
    end
