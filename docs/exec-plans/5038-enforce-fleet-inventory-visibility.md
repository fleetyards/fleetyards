...
- **2026-09-19** A second review pass found the read gate was too broad: `set_fleet_inventory` runs before `update?`/`destroy?`, and the privilege lists are independent — a role can carry `fleet:inventories:update` without `:read` — so a read-gated lookup answered 404 for a write the record rule allows. The scope is now two: `:visible` (visibility alone, for lookups that authorize afterwards) and the default (visibility behind the read gate, for lists). Both raise without a fleet context; putting the guard only in `:visible` let the default answer `none` instead.
- **2026-09-19** Declined, with reasons: a cross-fleet far end is not filtered. `visible_to?` takes a membership *of that fleet*, so visibility is a within-fleet setting and undefined for a counterparty — who learns of the inventory by receiving its goods, not through the rule. Filtering on "is visible to me" rather than "is not hidden from me" would also drop every cross-party transfer, a user inventory on the far end included.
