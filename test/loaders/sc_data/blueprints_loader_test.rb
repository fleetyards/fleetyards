...
module ScData
  module Loader
    class BlueprintsLoaderTest < ActiveSupport::TestCase
      ...
      # The `#one` tests below hand the loader a payload of their own and never
      # walk a tree, so this only has to be pointed somewhere harmless for them.
      # The three that do walk one read the fixture export.
      def loader
        fixture_loader(::ScData::Loader::BlueprintsLoader)
      end

      # `Blueprint#build` resolves against the configured source. These records
      # came from a fixture load, so the build they carry names the fixture
      # environment and the association finds nothing.
      def fixture_build(blueprint)
        blueprint.builds.current(fixture_source).first
      end

      # `retire_absent_builds` scopes to its loader's own environment, and the
      # factory writes a build in the configured one. These two tests hand it
      # rows directly rather than loading a tree, so they need a loader that
      # names the same environment the factory did -- which reads no files.
      def configured_loader
        ::ScData::Loader::BlueprintsLoader.new
      end
      ...
      test "#all retires a blueprint the export no longer carries" do
        ...
        retired = create(:blueprint, :without_build, sc_key: "bp_craft_gone", version: fixture_source.version)
        retired.builds.create!(
          environment: fixture_source.environment,
          version: fixture_source.version,
          **retired.attributes.symbolize_keys.slice(*BlueprintBuild::FACTS)
        )

        loader.all

        assert_nil retired.reload.version
        assert_nil fixture_build(retired)
      end
      ...
      test "#one links a component output by its ref" do
        ...
        assert_equal component, fixture_build(blueprint).craftable
      end
      ...
      test "#one resolves a cost option to its commodity" do
        ...
        option = fixture_build(blueprint).cost_slots.first.options.first
      ...
      test "dropping a build takes its recipe with it" do
        ...
        configured_loader.retire_absent_builds(BlueprintBuild, :blueprint_id, [create(:blueprint).id])
      ...
      test "#one keeps every segment of a piecewise ramp" do
        ...
        modifiers = fixture_build(blueprint).cost_slots.first.modifiers
      ...
      test "#one writes the org, the mission and the standing band" do
        ...
        source = fixture_build(blueprint).sources.first
      ...
      test "#one writes each distinct source once" do
        blueprint = loader.one(blueprint_data(ref: POOLED_REF))
        sources = fixture_build(blueprint).sources.map do |source|
      ...
      test "#one leaves a blueprint no pool names without sources" do
        ...
        assert_empty fixture_build(blueprint).sources
      ...
      test "#one replaces the sources rather than adding to them" do
        blueprint = loader.one(blueprint_data(ref: POOLED_REF))
        count = fixture_build(blueprint).sources.count

        assert_operator count, :>, 0

        loader.one(blueprint_data(ref: POOLED_REF))

        assert_equal count, fixture_build(blueprint).reload.sources.count
      end
