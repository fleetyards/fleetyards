...
module ScData
  module Loader
    class CommoditiesLoaderTest < ActiveSupport::TestCase
      ...
      # The game ships these as vectors, which ActiveStorage will neither serve
      # inline nor variant, so the parser rasterizes them on the way in and what
      # a commodity ends up with is a picture like any other.
      test "#all attaches the icon the parser carried over" do
        ...
        assert_equal "inv_icon_gold.png", image.filename.to_s
        assert_equal "image/png", image.content_type
        assert_predicate image, :representable?
      end
