...
test.describe("EmbedV2", () => {
  ...
  test("Default Workflow", async ({ page }) => {
    ...
    await expect(page.getByTestId("model-600i-explorer")).toHaveCount(
      1,
      EMBED_TIMEOUT,
    );

    await expect(
      page.getByTestId("model-600i-explorer").getByTestId("top-metrics"),
    ).toBeVisible();
    ...
    await expect(page.getByTestId("model-600i-explorer")).toHaveCount(2);
    ...
    await expect(page.getByTestId("model-600i-explorer")).toHaveCount(1);
  });
});
