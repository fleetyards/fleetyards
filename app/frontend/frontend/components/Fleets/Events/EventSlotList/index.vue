...
import {
  type FleetEventSlot,
  useCreateFleetEventSlot,
  useUpdateFleetEventSlot,
  useDestroyFleetEventSlot,
  useSortFleetEventSlots,
} from "@/services/fyApi";
import Sortable from "sortablejs";

type SlottableType = "FleetEventTeam" | "FleetEventShip";

type Props = {
  slottableType: SlottableType;
  slottableId: string;
  slots: FleetEventSlot[];
  editable?: boolean;
};
...
const localSlots = ref<FleetEventSlot[]>([]);
...
const createMutation = useCreateFleetEventSlot();
const updateMutation = useUpdateFleetEventSlot();
const destroyMutation = useDestroyFleetEventSlot();
const sortMutation = useSortFleetEventSlots();

const startEdit = (slot: FleetEventSlot) => {
...
const saveEdit = async (slot: FleetEventSlot) => {
  ...
  await updateMutation
    ...
    .catch(() =>
      displayAlert({ text: t("messages.fleets.eventSlot.update.failure") }),
    );
...
const submitNew = async () => {
  ...
  await createMutation
    ...
    .catch(() =>
      displayAlert({ text: t("messages.fleets.eventSlot.create.failure") }),
    );
};

const removeSlot = async (slot: FleetEventSlot) => {
  await destroyMutation
    ...
    .catch(() =>
      displayAlert({ text: t("messages.fleets.eventSlot.destroy.failure") }),
    );
};
  ...
  sortable = Sortable.create(container.value, {
    ...
    onEnd: () => {
      ...
      localSlots.value = order
        ...
        .filter(Boolean) as FleetEventSlot[];
