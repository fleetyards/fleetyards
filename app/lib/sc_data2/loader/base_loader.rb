  ...
  module Loader
    class BaseLoader
      ...
      def self.run_all(sc_version: nil)
        sc_version ||= Rails.configuration.app.sc_data2_sc_version

        ::ScData2::Loader::ManufacturersLoader.new(sc_version:).run
        ::ScData2::Loader::ItemsLoader.new(sc_version:).run
        ::ScData2::Loader::ModelsLoader.new(sc_version:).run
        ::ScData2::Loader::ModelModulesLoader.new(sc_version:).run
      end

      def initialize(sc_verison: nil)
        self.sc_version = sc_version || Rails.configuration.app.sc_data2_sc_version
      end
      ...
      private def update_loadout(parent, loadout, default_loadout = nil)
        loadout.each do |item|
          ...
          if hardpoint.component.blank? && hardpoint.hardpoints.blank?
            hardpoint.destroy!
          end
        end
      end
