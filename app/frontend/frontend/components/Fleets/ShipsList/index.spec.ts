 ...
 * reads its page out of `usePagination`, and its params out of the page, and
 * its queries out of the params. Get that order wrong and setup throws before
 * anything renders -- which is how the squadron ships view shipped broken
 * once. Mounting is the check; what it draws is covered elsewhere.
 */
...
vi.mock("@/services/fyApi", async (importOriginal) => ({
  ...
  useFleetSquadronVehicles: emptyQuery,
  useFleetSquadronVehiclesStats: emptyQuery,
  useFleetSquadronModelCounts: emptyQuery,
}));
...
const mountWith = (props: { fleet: Fleet; squadron?: FleetSquadron }) =>
  mount(Component, {
    props,
    shallow: true,
...
describe("FleetShipsList", () => {
  ...
  it("mounts scoped to a squadron", () => {
    expect(mountWith({ fleet, squadron }).exists()).toBe(true);
  });
});
