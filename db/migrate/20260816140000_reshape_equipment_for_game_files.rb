class ReshapeEquipmentForGameFiles < ActiveRecord::Migration[8.1]
  def up
    # The table holds rows entered by hand through admin forms that were retired
    # years ago. They are stale and largely wrong, so they are dropped rather
    # than migrated -- the game files replace them wholesale. Their prices go
    # with them: item_prices is polymorphic, and rows left pointing at a deleted
    # equipment id would fail to resolve.
    #
    # `down` restores the columns but cannot bring these rows back.
    execute("DELETE FROM item_prices WHERE item_type = 'Equipment'")
