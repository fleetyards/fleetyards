# frozen_string_literal: true

require_relative "../../../openapi_helper"

class Api::V1::PublicFleetsVehiclesEmbedTest < ActionDispatch::IntegrationTest
  include OpenapiRuby::Adapters::Minitest::DSL

  openapi_schema :"v1/schema"

  api_path "/public/fleets/{fleetSlug}/vehicles/embed" do
    parameter name: "fleetSlug", in: :path, schema: {type: :string}, description: "Fleet slug"

    get("Public Fleet Vehicles Embed for the Fleetyards Widget") do
      operationId "publicFleetVehiclesEmbed"
      tags "Fleets"
      produces "application/json"

      parameter name: "q", in: :query,
        schema: {
          type: :object,
          "$ref": "#/components/schemas/FleetVehicleQuery"
        },
        style: :deepObject,
        explode: true,
        required: false

      response(200, "successful") do
        schema type: :array, items: {"$ref": "#/components/schemas/VehiclePublic"}
      end

      response(404, "not found") do
        schema "$ref": "#/components/schemas/StandardError"
      end
    end
  end

  setup do
    Sidekiq::Testing.inline!
  end

  teardown do
    Sidekiq::Testing.fake!
  end

  test "GET /public/fleets/:fleetSlug/vehicles/embed returns vehicle list" do
    member = create(:user, vehicle_count: 3)
    fleet = create(:fleet, public_fleet: true, members: [member])

    assert_api_response :get, 200, path_params: {fleetSlug: fleet.slug} do
      assert_equal 3, parsed_body.count
    end
  end

  test "GET /public/fleets/:fleetSlug/vehicles/embed filters by modelNameCont" do
    member = create(:user, vehicle_count: 3)
    fleet = create(:fleet, public_fleet: true, members: [member])

    assert_api_response :get, 200,
      path_params: {fleetSlug: fleet.slug},
      params: {q: {"modelNameCont" => member.vehicles.first.model.name}} do
      assert_equal 1, parsed_body.count
      assert_equal member.vehicles.first.model.name, parsed_body.first.dig("model", "name")
    end
  end

  test "GET /public/fleets/:fleetSlug/vehicles/embed returns 404 for non-public fleet" do
    fleet = create(:fleet, public_fleet: false)

    assert_api_response :get, 404, path_params: {fleetSlug: fleet.slug}
  end

  test "GET /public/fleets/:fleetSlug/vehicles/embed returns 404 for unknown slug" do
    assert_api_response :get, 404, path_params: {fleetSlug: "unknown-fleet"}
  end
end
