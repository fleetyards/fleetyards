...
import {
  ...
  usePublicFleetModelsByClassification as usePublicFleetModelsByClassificationQuery,
  usePublicFleetModelsBySize as usePublicFleetModelsBySizeQuery,
  usePublicFleetModelsByProductionStatus as usePublicFleetModelsByProductionStatusQuery,
  usePublicFleetModelsByManufacturer as usePublicFleetModelsByManufacturerQuery,
  usePublicFleetVehiclesByModel as usePublicFleetVehiclesByModelQuery,
  type Fleet,
} from "@/services/fyApi";
...
const { data: vehicleStats } = usePublicFleetVehiclesStatsQuery(
  props.fleet.slug,
);

const { data: memberStats } = usePublicFleetMembersStatsQuery(props.fleet.slug);

const { data: modelsByClassificationOptions, ...modelsByClassificationStatus } =
  usePublicFleetModelsByClassificationQuery(props.fleet.slug);

const { data: modelsBySizeOptions, ...modelsBySizeStatus } =
  usePublicFleetModelsBySizeQuery(props.fleet.slug);

const {
  data: modelsByProductionStatusOptions,
  ...modelsByProductionStatusStatus
} = usePublicFleetModelsByProductionStatusQuery(props.fleet.slug);

const { data: modelsByManufacturerOptions, ...modelsByManufacturerStatus } =
  usePublicFleetModelsByManufacturerQuery(props.fleet.slug);

const { data: vehiclesByModelOptions, ...vehiclesByModelStatus } =
  usePublicFleetVehiclesByModelQuery(props.fleet.slug);
