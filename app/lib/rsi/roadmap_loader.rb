...
module Rsi
  class RoadmapLoader < ::Rsi::BaseLoader
    ...
    def load_roadmap_data
      return JSON.parse(File.read(json_file_path))['data']['releases'] if (Rails.env.test? || Rails.secrets.ci || Rails.secrets.rsi_load_from_file) && File.exist?(json_file_path)
    ...
    private def roadmap_maintenance_on?
      return false if Rails.env.test? || Rails.secrets.ci || Rails.secrets.rsi_load_from_file
      ...
      data.each do |release|
        release['cards'].each do |card|
          ...
          if item.store_image.blank?
            ...
            if image_url.present? && !Rails.env.test? && !Rails.secrets.ci && !Rails.secrets.rsi_load_from_file
