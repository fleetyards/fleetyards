...
class TradeRoutesWorker
  ...
  def perform
    ...
    ShopCommodity.commodity.where.not(sell_price: nil).find_each do |origin|
      ShopCommodity.commodity.where(commodity_item_id: origin.commodity_item_id).where.not(buy_price: nil).find_each do |destination|
