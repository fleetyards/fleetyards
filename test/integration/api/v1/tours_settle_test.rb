...
class Api::V1::ToursSettleTest < ActionDispatch::IntegrationTest
  ...
  api_path "/tours/{slug}/settle" do
    ...
    put("Settle Tour") do
      ...
      response(409, "cannot be settled") do
        schema ::Shared::V1::Schemas::StandardError
      end
    end
  end
  ...
  # Settling again re-ran PayoutLedger#settle!, whose first statement deletes
  # the transfers -- taking every confirmation people had ticked off with them.
  test "PUT refuses a second settle and keeps the confirmations" do
    sign_in @organiser

    assert_api_response :put, 200, path_params: {slug: @tour.slug}
    transfer = @ledger.reload.payout_transfers.first
    transfer.confirm!(@organiser)

    assert_api_response :put, 409, path_params: {slug: @tour.slug}

    assert_equal 1, @ledger.reload.payout_transfers.count
    assert_predicate transfer.reload, :confirmed?
  end

  test "PUT refuses to settle a cancelled tour" do
    @tour.cancel!
    sign_in @organiser

    assert_api_response :put, 409, path_params: {slug: @tour.slug}
    assert_equal "cancelled", @tour.reload.status
  end
end
