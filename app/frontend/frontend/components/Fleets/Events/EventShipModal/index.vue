...
import {
  ...
  useCreateFleetEventShip,
  useUpdateFleetEventShip,
} from "@/services/fyApi";
...
const createMutation = useCreateFleetEventShip();
const updateMutation = useUpdateFleetEventShip();
...
const onSubmit = handleSubmit(async (values) => {
  ...
  await mutation
    .then(() => {
      displaySuccess({
        text: isEdit.value
          ? t("messages.fleets.eventShip.update.success")
          : t("messages.fleets.eventShip.create.success"),
      });
    ...
    .catch(() => {
      displayAlert({
        text: isEdit.value
          ? t("messages.fleets.eventShip.update.failure")
          : t("messages.fleets.eventShip.create.failure"),
      });
    })
    .finally(() => {
...
<template>
  <Modal
    :title="
      isEdit
        ? t('headlines.fleets.events.editShip')
        : t('headlines.fleets.events.addShip')
    "
  >
    <form id="event-ship-form" @submit.prevent="onSubmit">
      <div class="flex">
        <BtnGroup>
          <Btn
