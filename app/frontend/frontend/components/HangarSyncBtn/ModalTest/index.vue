...
import testHtml2 from "./test2.html?raw";
...
const start = async () => {
  ...
  if (debug.value) {
    const items = new RSIHangarParser().extractPage(testHtml2);

    if (items) {
      pledges.value = [...pledges.value, ...items];
    }

    const items2 = new RSIHangarParser().extractPage(testHtml);

    if (items2) {
      pledges.value = [...pledges.value, ...items2];
