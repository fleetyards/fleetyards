...
//
// Once a holo has been measured, a difference from the game files is no longer
// worth flagging: `maxBoundingBoxSize` does not orient its axes consistently and
// names no configuration, so it is the weaker figure and disagreeing with it is
// the expected state. Only the warning goes -- the game-file value stays on the
// subline and stays appliable, because a holo measurement is not beyond doubt
// either (a hull rendered with its wings vertical sorts its axes wrongly).
