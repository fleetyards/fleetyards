...
- [x] Create migration to update `active_storage_attachments.name` from `new_X` to `X`:
...
- [x] Delete all uploaders (`app/uploaders/*.rb` — 9 files)
- [x] Delete `CarrierwaveToActiveStorageMigration` concern and remaining 3 migration tasks (already removed)
- [x] Delete `lib/carrier_wave/validations/active_model.rb`
- [x] Delete `config/initializers/carrierwave.rb`
- [x] Remove gems from Gemfile: `carrierwave`, `fog-aws`, `ssrf_filter`
- [x] Keep `mini_magick` and `image_processing` (used by Active Storage variants and compare_image)

## ~~Phase 4: Database Cleanup~~ ✅

- [x] Migration to remove `carrierwave_migrated_at` from all 11 tables
- [x] Migration to remove old string columns (`store_image`, `avatar`, `logo`, `background_image`, `import`, etc.)
- [x] Removed width/height columns (Active Storage metadata replaces them)
- [x] Removed `ignored_columns` declarations from all models
- [x] Updated Ransack attributes: removed width/height refs, added Active Storage ransackers for admin `_blank` filters

## Phase 5: Cleanup

- [ ] Remove AWS credentials for carrierwave (`carrierwave_cloud_key`, etc.) from Rails credentials via `rails credentials:edit`
- [x] Verify all image serving works via Active Storage URLs
