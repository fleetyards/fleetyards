...
class Api::V1::FleetsInventoryStockUpdateTest < ActionDispatch::IntegrationTest
  ...
  api_path "/fleets/{fleetSlug}/inventories/{fleetInventorySlug}/stock/{slug}" do
    ...
    patch("Update Fleet Inventory Stock Position") do
      ...
      request_body required: true, schema: ::V1::Schemas::Inputs::InventoryStockPositionInput
  ...
  test "PATCH records the rename against the signed-in officer" do
    ...
    versions = PaperTrail::Version.where(
      item_type: "FleetInventoryPosition", item_id: @inventory.positions.select(:id), event: "update"
    )
