...
Rails.application.config.after_initialize do
  ...
  # The Loader infers shared scope from component_scope_paths but doesn't
  # set _component_scopes_explicitly_set for shared components. This means
  # they're excluded when filtering by scope. Mark them as explicitly shared
  # so they're visible to all scopes (needed for security scheme resolution).
  shared_prefix = OpenapiRuby.configuration.component_scope_paths.find { |_, v| v.to_sym == :shared }&.first
  if shared_prefix
    OpenapiRuby::Components::Registry.instance.all_registered_classes.each do |klass|
      ...
      next unless klass._component_scopes.empty?

      source = klass.name&.underscore
      next unless source&.start_with?(shared_prefix.tr("/", "::").underscore.tr("::", "/"))
