...
class Api::V1::FleetsSquadronVehiclesExportTest < ActionDispatch::IntegrationTest
  include OpenapiRuby::Adapters::Minitest::DSL

  openapi_schema :"v1/schema"

  api_path "/fleets/{fleetSlug}/squadrons/{fleetSquadronSlug}/vehicles/export" do
    parameter name: "fleetSlug", in: :path, schema: {type: :string}, description: "Fleet slug"
    parameter name: "fleetSquadronSlug", in: :path, schema: {type: :string}, description: "Squadron slug"

    get("Fleet Squadron Vehicles Export") do
      operationId "fleetSquadronVehiclesExport"
      tags "FleetSquadrons"
  ...
  test "GET squadron vehicle export returns only the squadron members' ships" do
    sign_in @admin

    assert_api_response :get, 200, path_params: path_params do
      assert_equal 3, parsed_body.count
      assert_equal [@member.username], parsed_body.map { |entry| entry["username"] }.uniq
    end
  end

  test "GET squadron vehicle export is empty for a squadron with nobody in it" do
  ...
  test "GET squadron vehicle export returns 404 for an unknown squadron" do
    sign_in @admin

    assert_api_response :get, 404, path_params: {fleetSlug: @fleet.slug, fleetSquadronSlug: "no-such-wing"}
  end

  test "GET squadron vehicle export returns 401 when not signed in" do
