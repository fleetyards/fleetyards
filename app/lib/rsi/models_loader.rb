...
module Rsi
  class ModelsLoader < ::Rsi::BaseLoader
    ...
    def load_models
      return JSON.parse(File.read(json_file_path))['data'] if (Rails.env.test? || Rails.secrets.ci || Rails.secrets.rsi_load_from_file) && File.exist?(json_file_path)
    ...
    def load_buying_options(model)
      return if Rails.env.test? || Rails.secrets.ci || Rails.secrets.rsi_load_from_file
    ...
    private def load_store_image(model, media_data)
      ...
      return if store_image_url.blank? || Rails.env.test? || Rails.secrets.ci || Rails.secrets.rsi_load_from_file
