...
class Admin::Api::V1::ModelUpgradesLinkTest < ActionDispatch::IntegrationTest
  include OpenapiRuby::Adapters::Minitest::DSL

  openapi_schema :"admin/v1/schema"

  api_path "/model-upgrades/{id}/link" do
  ...
  test "PUT /model-upgrades/:id/link creates the upgrade kit" do
    model = create(:model)
    upgrade = create(:model_upgrade)
    sign_in @user

    assert_api_response :put, 200, path_params: {id: upgrade.id}, body: {modelId: model.id} do
      assert_equal 1, UpgradeKit.where(model_id: model.id, model_upgrade_id: upgrade.id).count
    end
  end

  test "PUT /model-upgrades/:id/link returns 401 when not signed in" do
    model = create(:model)
    upgrade = create(:model_upgrade)
  ...
  test "PUT /model-upgrades/:id/link returns 403 for admin without access" do
    model = create(:model)
    upgrade = create(:model_upgrade)
